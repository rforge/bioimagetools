K.cross.3D.envelope<-function(X,Y,Z,X2,Y2,Z2,range,bw.int,psz,width,multicore=FALSE,N=100,use.intensity=TRUE)
  {
K.envelope<-c()
...
shuffle<-function(i)
  {
    ...
    K.temp<-K.cross.3D(X,Y,Z,XX,YY,ZZ,psz=psz,width=width,intensity=intensity.red,intensity2=intensity.green,multicore=multicore)
    return(K.temp$y)
  }
...
K<-K.cross.3D(X+range,Y+range,Z+range,X2+range,Y2+range,Z2+range,psz=psz,width=width,intensity=intensity.red,intensity2=intensity.green,multicore=multicore)
